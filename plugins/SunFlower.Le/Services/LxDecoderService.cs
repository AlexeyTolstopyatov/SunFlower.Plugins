...
public partial class LxDecoderService
{
    private readonly LxDumpManager _dump;
    ...
    public LxDecoderService(string filePath, LxDumpManager dump)
    {
        _filePath = filePath;
        _dump = dump;
        _mainObj = (int)_dump.LxHeader.e32_startobj;
        _mainEip = _dump.LxHeader.e32_eip;
        _pageSize = (int)_dump.LxHeader.e32_pagesize;
        _pageShift = (int)_dump.LxHeader.e32_pageshift;
    }
    ...
    public string[] Disassemble()
    {
        ...
        _results.AddRange([
            $"; SunFlower.LE.dll for Linear eXecutable {moduleName} (bases on IBM OS/2 LX docs 2001-06-14 rev 11)",
    ...
    private void BuildFixupSymbolMap()
    {
        ...
        for (var oi = 0; oi < _dump.Objects.Length; oi++)
        {
            ...
            var startLogical = (int)obj.PageMapIndex; // 1-based per LX spec
    ...
    private byte[]? BuildObjectBytesByPageIndex(Object obj)
    {
        ...
        {
            var page = _dump.Pages[startLogical + i];
            var rawFlags = page.Flags;
            var pageType = (byte)(rawFlags & 0x03);

            // LX: 0=Legal physical page, 1=Iterated, 2=Invalid, 3=ZeroFilled
